:scobc_a1_dev_overlay_link: https://github.com/spacecubics/scobc-a1-sample/blob/main/boards/shields/scobc_a1_dev/scobc_a1_dev.overlay
...
Referring to the {scobc_a1_dev_overlay_link}[DTS overlay file for Development Board], it can be seen that `GPIO15` of 
the GPIO IP core is defined with the device name `user_led_0`. +
Additionally, an `led0` is assigned using the alias, which is used by Zephyr's blinky sample. However, this alias is 
not mandatory.
...
Using the *GPIO* driver for the SC-OBC Module A1, this application reads the state of the DIP switch SW1[9] on the _Development Board_ and displays its state on the console.  
This sample demonstrates how to read a GPIO configured as input.

The sample application is available at:
https://github.com/spacecubics/scobc-a1-sample/tree/main/samples/dip_switch[DIP switch sample].
...
The DIP switch (_SW1_) on the Development Board is connected to GPIO lines of the AMD AXI GPIO IP core.  
In this sample, **SW1[9] of the DIP switch** is used as input.

The corresponding GPIO line and its mapping are defined in the DTS overlay file for the Development Board.

Referring to the  {scobc_a1_dev_overlay_link}[DTS overlay file for Development Board], it can be seen that `GPIO14` of 
the GPIO IP core is defined with the device name `user_sw_1`. +
Additionally, an `sw1` is assigned using the alias, which is used by the DIP Switch sample. However, this alias is not mandatory.
...
The GPIO is configured to its initial state as `GPIO_INPUT`. If the configuration fails, an error code will be returned.
...
CAUTION: To read the DIP switch state, the corresponding GPIO line must not be used by another peripheral and the correct DIP switch bit must be toggled manually.
...
After power-cycle the SC-OBC Module A1, you should see the DIP switch state displayed on the serial console when toggling SW1[9].
